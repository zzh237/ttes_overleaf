...
n^{-\beta}.
\end{equation}
Thus, in the count-only experiment, every fixed finite $n=2N$ leaves an identification width of order $n^{-\beta}$ even as $Q\to\infty$.  A random-permutation embedding in \cref{thm:observed-transfer,cor:general-lower-transfer} shows that the same order is a uniform finite-sample minimax lower bound when literal question strings are observed and the supremum ranges jointly over the question distribution and the question-to-advantage map.  This is not pointwise nonidentifiability for one fixed finite question population: the least-favorable population may become more diffuse as $Q$ grows.
...
There are two models and two answers \aed{$\{\mathrm{gold}, \mathrm{wrong}\}$}.  Each question $q$ has a latent signed advantage $Z_q\in[-1,1]$ and
\begin{equation}
...
\qquad \sigma\in\{-1,+1\},
\]
every observed count is deterministically $0$ or $n$.  The problem is only to detect an imbalance of order $Q^{-1/2}$ between the two question types; finite-generation noise plays no role.  A Le Cam proof for the count-only experiment is given in \cref{app:finite-lower}.  The observed-question and general-$K$ versions follow from \cref{thm:observed-transfer,cor:general-lower-transfer}.
...
\subsection{Observed finite strings and minimax transfer}
\label{sec:observed-transfer}

The count-only experiment in \eqref{eq:scalar-marginal-experiment} deliberately discards the question identities.  This is harmless for upper bounds, because a procedure in the original experiment can always ignore the questions.  It is not harmless for lower bounds: an estimator that observes $q_j$ may use the question text, or may recognize that two observations correspond to the same question, to learn something about the latent advantage $Z(q_j)$.  We now show that this extra information does not improve the minimax rate over a broad nonparametric class in which both the population distribution of questions and the question-to-advantage map are unrestricted, subject only to the pushforward margin condition.

To model literal text questions, let $\mathcal X$ be a countably infinite question space with the discrete sigma-field; the canonical example is the set $\Sigma^\star$ of finite strings over a finite alphabet $\Sigma$.  The parameter is a pair $(\calQ,Z)$, where $\calQ$ is a probability distribution on $\mathcal X$ and $Z:\mathcal X\to[-1,1]$ is an arbitrary map.  Define the observed-question parameter class
\begin{equation}
\label{eq:observed-parameter-class}
...
\right\}.
\end{equation}
Here $\mathcal P(\mathcal X)$ denotes the probability measures on the discrete space $\mathcal X$, and $Z_\#\calQ$ is the pushforward law of $Z(q)$ for $q\sim\calQ$.  For $(\calQ,Z)\in\mathfrak P^{\mathrm{obs}}_{\beta,L}$, the observed-question antisymmetric experiment is
\begin{equation}
\label{eq:full-observed-scalar}
q_1,\dots,q_Q\overset{\mathrm{iid}}{\sim}\calQ,
\qquad
S_j\mid q_j
\sim
...
:=
\inf_{\widehat\sigma}
\sup_{(\calQ,Z)\in\mathfrak P^{\mathrm{obs}}_{\beta,L}}
\E_{\calQ,Z}\!\left[
|\theta_{\calQ,Z}|\,
\one\{\widehat\sigma\theta_{\calQ,Z}<0\}
\right],
\end{equation}
...
The supremum in \eqref{eq:minimax-observed} ranges jointly over the question distribution and the advantage map.  This is the usual distribution-free minimax formulation: the parameter class is fixed once and for all, while the least-favorable member selected from it may depend on $(Q,n)$.  In particular, the class contains populations that are uniform on arbitrarily large finite collections of distinct question strings.

Under any $(\calQ,Z)$ in the observed class, the marginal count law is $K_nG$ with $G=Z_\#\calQ\in\calG_{\beta,L}$.  Hence every count-only rule is also admissible in \eqref{eq:minimax-observed}, and
\begin{equation}
...
We first record two ingredients.  Their proofs are given in Appendix~\ref{app:observed-transfer}.
...
\begin{lemma}[Random-permutation embedding on a finite question population]
\label{lem:permutation-embedding}
Let $x_1,\dots,x_M$ be distinct questions in $\mathcal X$, let
\[
\calQ_M:=\frac1M\sum_{r=1}^M\delta_{x_r},
\]
and let $G_M=M^{-1}\sum_{r=1}^M\delta_{z_r}$ be an equal-weight measure.  Draw a uniform random permutation $\Pi$ of $[M]$ and define the deterministic map
\[
Z_\Pi(x_r)=z_{\Pi(r)},
\qquad r\in[M],
\]
with arbitrary values outside $\{x_1,\dots,x_M\}$.  Let $\overline{\mathbb P}^{Q,n}_{G_M,M}$ be the law of the full sample in \eqref{eq:full-observed-scalar} after averaging over $\Pi$, with question distribution $\calQ_M$.  Then
\begin{equation}
\label{eq:permutation-TV}
\TV\!\left(
\overline{\mathbb P}^{Q,n}_{G_M,M},
\calQ_M^{\otimes Q}\otimes(K_nG_M)^{\otimes Q}
\right)
\le
3\alpha_{Q,M},
\qquad
...
The comparison law in \eqref{eq:permutation-TV} retains the observed question strings, but makes them independent of the counts.  Averaging over the unknown permutation makes the strings ancillary except when a question identity or an independently assigned latent index is repeated.  The discrepancy is therefore of order $Q^2/M$.  Because the fixed class \eqref{eq:observed-parameter-class} contains uniform distributions on finite populations of every size $M$, the lower-bound proof may choose $M\gg Q^2$ without making the parameter class depend on the sample size.
...
The lower bound is a least-favorable-prior argument supported entirely on valid deterministic maps on finite strings.  Starting from a reflected scalar hard pair, Lemma~\ref{lem:equal-weight-discretization} replaces the two mixing laws by equal-weight finite measures without changing the margin constant.  Their atoms are then assigned by independent uniform permutations to the same large finite collection of observed questions under the two hypotheses.  Lemma~\ref{lem:permutation-embedding} compares the resulting prior-averaged full-data laws with the corresponding count-only product laws, and Le Cam's inequality transfers the target separation.  The full proof, including the minimax quantifiers, is in Appendix~\ref{app:observed-transfer}.
...
Fix $K\ge2$ and $A\ge2$.  Consider any full-model minimax class on the countable question space $\mathcal X$ that contains the following subclass: $(\calQ,Z)$ ranges over \eqref{eq:observed-parameter-class}; there are two answers; models 1 and 2 obey \eqref{eq:antisymmetric}; and every model $i\ge3$ returns either answer with probability $1/2$.  Then, for every estimator $\widehat w\in\Delta_K$ based on the observed questions and all raw generations,
\begin{equation}
\label{eq:general-lower-transfer}
\inf_{\widehat w}
\sup_{(\calQ,Z)\in\mathfrak P^{\mathrm{obs}}_{\beta,L}}
\E_{\calQ,Z}\!\left[
R_{\calQ,Z}(\widehat w)-\inf_{w\in\Delta_K}R_{\calQ,Z}(w)
\right]
\ge
...
\end{equation}
Consequently, every scalar lower bound proved below---including the root-$Q$, moment-floor, and pooled rare-boundary bounds---is a valid minimax lower bound for any larger unrestricted ensemble class containing this submodel.
\end{corollary}
...
M_q(w)=(w_1-w_2)Z(q).
\]
An optimal vertex is $e_1$ when $\theta_{\calQ,Z}>0$ and $e_2$ when $\theta_{\calQ,Z}<0$; at either choice,
\[
\Pp_{q\sim\calQ}\{0<M_q(w^\star)\le t\}
\le
(Z_\#\calQ)([-t,t])
\le Lt^\beta.
\]
Thus the embedded environments satisfy the one-sided comparator margin condition used in the general theorem.  Given an arbitrary weight estimator, define the induced scalar decision to be $+1$ when $\widehat w_1>\widehat w_2$ and $-1$ otherwise.  If $\widehat w_1\ne\widehat w_2$, choosing the wrong side incurs clean excess risk exactly $|\theta_{\calQ,Z}|$.  If $\widehat w_1=\widehat w_2$, every question is tied and the clean excess risk is at least $|\theta_{\calQ,Z}|$ whenever the fixed tie decision is wrong.  Hence, pointwise in the data,
\[
R_{\calQ,Z}(\widehat w)-\inf_wR_{\calQ,Z}(w)
\ge
|\theta_{\calQ,Z}|\one\{\widehat\sigma\theta_{\calQ,Z}<0\}.
\]
...
\begin{remark}[Scope and interpretation of the transfer]
\label{rem:transfer-scope}
The minimax supremum ranges jointly over $\calQ$ and $Z$.  The parameter class is fixed and contains every finite support size $M$; only the least-favorable pair selected from it depends on $(Q,n)$.  Thus the transferred $n^{-\beta}$ bound is a uniform finite-sample minimax obstruction over an unrestricted family of text populations.  It is not a claim of pointwise nonidentifiability for one fixed finite question population as $Q\to\infty$.

For a fixed finite support, repeated identities can genuinely help even when the text has no semantic relation to the advantage.  The reason is that repetition identifies which noisy counts share the same conditional parameter.  For example, when $n=2$, the reflected measures
\[
G_+=\frac34\delta_{1/3}+\frac14\delta_{-1},
\qquad
G_-=\frac34\delta_{-1/3}+\frac14\delta_{1}
\]
have the same first two moments and hence the same count-only law, while $\theta(G_+)=1/2$ and $\theta(G_-)=-1/2$.  If these four equal masses are assigned to four observed questions and the same questions recur, their conditional binomial distributions can eventually be estimated separately, revealing the assignment.  The lower-bound construction avoids this extra information at each finite $Q$ by choosing $M\gg Q^2$.

A technically elegant alternative fixes the nonatomic population $([0,1],\lambda)$ and places all complexity in an arbitrary measurable map $Z$.  The proof then assigns equal-weight hard instances to equal-length intervals, exactly as in the finite-string construction.  This fixed-$\calQ$ formulation gives the same rate and is canonical among atomless standard probability spaces, but the countable formulation above is closer to literal text questions.  Finally, any structural relation connecting semantic features of $q$ to $Z(q)$ can make the observed questions informative and may improve the rate; that is a different, covariate-assisted problem.
\end{remark}

\subsection{The count-only infinite-question identification floor}

\ced{The moment equivalence in \cref{lem:moment-equivalence} leaves open how accurately $\theta(G)$ can be recovered from those moments over the margin class.  The next theorem gives the sharp answer.}
...
Moreover, \cref{thm:observed-transfer,cor:general-lower-transfer} imply the same order as a uniform minimax lower bound for the observed-question antisymmetric model and every larger unrestricted general ensemble class containing it.
\end{theorem}
...
\end{equation}
\end{theorem}
The construction and its Le Cam argument are given in \cref{app:finite-lower}.  The observed-question and general-ensemble versions follow from \cref{thm:observed-transfer,cor:general-lower-transfer}.
...
The scalar count experiment is not literally the same as the original observed-question experiment.  \Cref{thm:observed-transfer} closes this gap on a countably infinite question space, such as the set of finite strings, by choosing a large finite population of distinct questions and randomly permuting a finite approximation of the scalar hard instance across them.  The prior is supported on deterministic maps, and the observed strings are ancillary up to an explicit $O(Q^2/M)$ collision error.  \Cref{cor:general-lower-transfer} then embeds this observed-question model into every $K\ge2$, $A\ge2$ ensemble class containing the resulting unrestricted answer kernels.  Thus the root-$Q$, $N^{-\beta}$, and pooled rare-boundary lower bounds all transfer.  This conclusion can fail under additional structure that makes question features predictive of the latent advantages.
...
The scalar antisymmetric model shows that this is not the end of the story.  When evidence can be pooled across many questions, the problem becomes indirect estimation of \ced{a linear sign-imbalance functional} through a binomial-mixture channel.  The exact scalar minimax difficulty is governed by a Hellinger modulus, and the count-only channel has an $N^{-\beta}$ moment-identification width rather than an $N^{-\beta/2}$ floor.  A random-permutation embedding on a large finite population of distinct question strings proves that the same lower bounds remain valid when question identities are observed and in larger unrestricted ensemble classes.  The two rigorous endpoint regimes already reveal a qualitative transition between question-limited learning and population-level moment inversion.  Determining the exact spectral interpolation, and extending the matching upper theory to genuinely multivariate ensemble weights, are natural next problems.
...
This appendix proves Lemmas~\ref{lem:equal-weight-discretization} and~\ref{lem:permutation-embedding}, and Theorem~\ref{thm:observed-transfer}.  The argument is fully frequentist: the random permutation is used only to define a prior supported on deterministic question-to-advantage maps, and the minimax risk is lower-bounded by the corresponding Bayes risk.
...
Write $q_j=x_{R_j}$, where $R_1,\dots,R_Q$ are iid uniform on $[M]$ under both laws in \eqref{eq:permutation-TV}.  Augment the prior-averaged permutation experiment by the latent indices
\[
J_j=\Pi(R_j).
\]
...
Let $A$ be the event that $R_1,\dots,R_Q$ are all distinct.  In the permutation experiment, conditional on $A$, the vector $(J_1,\dots,J_Q)$ is a uniformly ordered sample without replacement from $[M]$ and is independent of the ordered distinct vector $(R_1,\dots,R_Q)$.  Let $E$ be the event, in the comparison experiment, that both the $R_j$'s and the $J_j$'s are distinct.  Conditional on $E$, the two ordered vectors are independent uniformly ordered samples without replacement.  The conditional count channels are identical.  Hence the two augmented conditional laws agree exactly:
\[
\mathcal L_{\mathrm{perm}}((R,J,S)\mid A)
=
\mathcal L_{\mathrm{iid}}((R,J,S)\mid E).
\]
...
\begin{align*}
\TV(\mathcal L_{\mathrm{perm}}(R,J,S),
     \mathcal L_{\mathrm{iid}}(R,J,S))
&\le
\pr_{\mathrm{perm}}(A^c)
+
\pr_{\mathrm{iid}}(E^c)\\
...
&\le3\alpha_{Q,M}.
\end{align*}
Applying the deterministic map $R_j\mapsto x_{R_j}$ and then discarding the latent indices $J_j$ are Markov kernels and cannot increase total variation.  The remaining iid comparison law is exactly $\calQ_M^{\otimes Q}\otimes(K_nG_M)^{\otimes Q}$, proving \eqref{eq:permutation-TV}.
\end{proof}
...
Choose any $M$ distinct strings $x_1,\dots,x_M\in\mathcal X$ and let $\calQ_M$ be uniform on them.  For each sign, place the uniform-permutation prior of Lemma~\ref{lem:permutation-embedding} on the deterministic maps whose pushforward under $\calQ_M$ is $G^{(M)}_+$ or $G^{(M)}_-$.  Denote the resulting prior-averaged full-data laws by $\overline{\mathbb P}_+$ and $\overline{\mathbb P}_-$.  Every pair $(\calQ_M,Z)$ in the support belongs to $\mathfrak P^{\mathrm{obs}}_{\beta,L}$ and has target respectively $+a_M$ or $-a_M$, where $a_M:=\theta(G^{(M)}_+)\ge\Delta/4$.  Combining Lemma~\ref{lem:permutation-embedding} with \eqref{eq:transfer-product-TV},
\[
