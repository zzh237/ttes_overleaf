...
n^{-\beta}.
\end{equation}
Thus, in the count-only experiment, every fixed finite $n=2N$ leaves an identification width of order $n^{-\beta}$ even as $Q\to\infty$.  A random-permutation embedding in \cref{thm:observed-transfer,cor:general-lower-transfer} shows that the same order is a uniform minimax lower bound when the questions themselves are observed, and hence for the unrestricted general ensemble class.  In the observed-question formulation this is a nonuniform minimax obstruction over arbitrary question-to-advantage maps, rather than literal nonidentifiability of every fixed map.
...
To make the statement precise, fix the canonical nonatomic question population $([0,1],\mathcal B,\lambda)$, where $\lambda$ is Lebesgue measure.  Let
\[
\mathfrak Z_{\beta,L}
:=
...
The comparison law in \eqref{eq:permutation-TV} keeps the observed questions but makes them independent of the count-only observations.  Averaging over the unknown assignment makes the question labels ancillary except when two sampled questions fall in the same partition cell.  The probability of such a collision is of order $Q^2/M$, and $M$ is free to be arbitrarily large inside the fixed nonparametric parameter class.
...
\label{rem:nonatomic}
The nonatomic question population is used to produce arbitrarily fine equal-probability partitions while keeping the question distribution fixed.  The same argument on a finite uniform population of $M$ opaque labels has the explicit additional error $O(Q^2/M)$ in \eqref{eq:permutation-TV}.  Thus a fixed finite question population is a different asymptotic problem: once labels repeat often, question identity can help estimate their associated effects.  Likewise, structural assumptions connecting question covariates or text to $Z(q)$ can improve the rate.  The theorem concerns the broad nonparametric population model in which no such regularity is imposed.

There is also a useful distinction between the two experiments as $Q\to\infty$.  In the count-only experiment, $n^{-\beta}$ is a literal nonidentification width: two mixing laws can induce exactly the same count distribution.  In the observed-question model, \cref{thm:observed-transfer} gives the same lower bound uniformly for every finite $Q$, and therefore a nonvanishing minimax limit over the unrestricted class, but the least-favorable map may vary with $Q$.  The statement is a uniform minimax obstruction, not a claim that every fixed map remains unidentified from an infinite observed-question sample.
\end{remark}
