...
\quad j=1,\dots,Q,}
\end{equation}
\ced{By sufficiency, we may work with the reduced experiment in which only $S_1,\dots,S_Q$ are retained.}
...
As $Q\to\infty$, the observations identify only $K_nG$, equivalently the first $n$ moments of $G$. Consequently, they need not identify $G$, or even $\theta(G)$.  This is the  source of the finite-generation identification floor quantified below.

\begin{remark}[Observed questions and the scalar experiment]
\ced{The hierarchy in~\eqref{eq:scalar-reduced-experiment} is the exchangeable random-effects representation induced by sampling questions independently from $\calQ$: $Z_j=Z(q_j)$ is a question-specific latent effect, random through $q_j\sim\calQ$, and its population distribution is $G$.  This representation is exact after marginalizing the question identities.  The full experiment, however, also observes $q_j$, whereas the scalar experiment below retains only $S_j$.  Consequently, transferring a scalar lower bound to the full experiment requires an additional embedding argument showing that the question labels carry no useful information about their assigned latent advantages.  Because the full model imposes no structure on the map $q\mapsto Z(q)$, such an embedding may be obtained by randomly assigning a hard collection of latent advantages to otherwise uninformative question labels; a rigorous version requires a random-permutation or Yao argument.  Count-based upper bounds do not face this issue, since a procedure in the full experiment may simply ignore the question identities.}
\end{remark}
...
\subsection{The infinite-question identification floor}
